...
**Denoising autoencoder (DAE):** Consider a pair of clean and corrupted data $(\mathbf{x},\tilde{\mathbf{x}})$. The joint density of corrupted and uncorrupted samples $q(\tilde{\mathbf{x}},\mathbf{x})=q(\tilde{\mathbf{x}}|\mathbf{x})q_0(\mathbf{x})$. 

In DAE, ...., optimize the expected squared reconstruction:
...
**(Manifold hypothesis)** The data in real world tend to concentrate on low dimensional manifolds embedded in high dimensional space (ambient space), it brings two challenges to this generative process through score function: (1) undefined score function in ambient space; (2) scarcity of training data in low density region.
...
J_{NCSN}(\theta)= \min_\theta
   \sum_{i=1}^{N} \sigma_i^2  \mathbf{E}_{p_{data}(\bf x)}\mathbf{E}_{p_{\sigma_i}(\tilde{\bf x} \mid {\bf x})}\big[||{ {\bf s}_\theta(\tilde{\bf x}, \sigma_i) - \nabla_{\tilde{\bf x}} \log p_{\sigma_i}(\tilde{\bf x} \mid {\bf x})}||_2^2 \big]
\tag{1}
\end{equation}
$$
...
$$
\begin{equation}
{\bf x}_i^m = {\bf x}_i^{m-1} + \epsilon_i {\bf s}_\theta^*({\bf x}_i^{m-1}, \sigma_i) + \sqrt{2\epsilon_i} {\bf z}_i^m,\quad m=1,2,\cdots, M;\quad i=N,N-1,...,1
\tag{1}
\end{equation}
$$

where $\epsilon_i$ is the step size, ${\bf z}_{i}^m$ is the stand normal, $m$ indicates the Langevin sampling step index and $i$ is the noise level index.
...
J_{DDPM}(\theta)= \min_\theta
    \sum_{i=1}^{N} (1-\alpha_i) \mathbf{E}_{p_{data}d({\bf x})}\mathbf{E}_{p_{\alpha_i}(\tilde{\bf x} \mid \bf x)}[||{ {\bf s}_\theta(\tilde{\bf x}, i) - \nabla_{\tilde{\bf x}} \log p_{\alpha_i}(\tilde{\bf x} \mid {\bf x})}||_2^2]. 
\tag{3}
\end{equation}
$$
...
$$
\begin{equation}
    {\bf x}_{i-1} = \frac{1}{\sqrt{1-\beta_i}} ({\bf x}_i + \beta_i {\bf s}_{\theta^*}({\bf x}_i, i)) + \sqrt{\beta_i}{\bf z}_i, \quad i=N, N-1, \cdots, 1. 
\tag{4}
\end{equation}
$$
...
$$
\begin{equation}
p({\bf x}_i|{\bf x}_{i-1})=\mathcal{N}({\bf x}_i;\sqrt{1-\beta_i},\beta_i{\bf I})
\end{equation}
$$
